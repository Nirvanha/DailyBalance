...
interface ActionRecordDao {
    ...
    @Query("SELECT * FROM action_record WHERE type = :type AND timestamp BETWEEN :fromTimestamp AND :toTimestamp ORDER BY timestamp DESC")
    suspend fun getByTypeBetween(type: String, fromTimestamp: Long, toTimestamp: Long): List<ActionRecord>
    ...
    @Query("DELETE FROM action_record WHERE type = :type AND timestamp BETWEEN :fromTimestamp AND :toTimestamp")
    suspend fun deleteByTypeBetween(type: String, fromTimestamp: Long, toTimestamp: Long)

    @Query("DELETE FROM action_record WHERE id = :id")
